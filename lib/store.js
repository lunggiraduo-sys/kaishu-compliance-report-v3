...
function createStore({ dataDir, clock = () => new Date(), backupRetentionDays = 30 } = {}) {
  ...
  return {
    ...
    // 附件存储（本地文件系统）
    async saveAttachment(id, fileBuffer, originalName, mime) {
    ...
    async getAttachment(id) {
      ...
      const data = await fs.readFile(path.join(uploadDir, match));
      return { data, originalName: match, mime: 'application/octet-stream', size: data.length };
    },
  };
}
