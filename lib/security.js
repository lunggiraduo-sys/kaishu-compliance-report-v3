...
function createSessionCookie(name, token, { expiresAt, path = '/' } = {}) {
  ...
  return `${name}=${token}; Path=${path}; Expires=${expiry.toUTCString()}; HttpOnly; SameSite=None; Secure`;
}

function clearCookie(name, path = '/') {
  return `${name}=; Path=${path}; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=None; Secure`;
}
